...
- **Process Selection**: Users can choose from various types of random processes, starting with flipping a coin.
- **Parameter Adjustment**: Users can modify parameters such as the number of trials, probability distributions, and time intervals. For number of trials, the default is set to one, but users can add more trials as needed, keeping the already existing trials intact. Users will be able to reset parameters, resulting in the removal of all existing trials.
